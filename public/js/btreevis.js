...
tree.prototype.show = function (canvasId) {
	...
	if (this.length > maxDisplay) {
		...
		if (!this.eof) {
			treeHTML += ' Current key: '+this.leaf.keyval[this.item];
		}
		treeHTML += '<br>\r\n';
		return treeHTML;
	}
...
tree.prototype.listNode = function (ptr,lvl) {
	...
	for (let i = 0, len = this.maxkey; i < len; i++) {
		if (ptr == this.leaf && i == this.item) {
			treeHTML += '<td class="here">';
		} else {                               
			treeHTML += '<td>';
		}
		if (i < ptr.keyval.length) {
			treeHTML += ptr.keyval[i]+'</td>';
		}
		else {
			treeHTML += '&nbsp;</td>';
		}
...
tree.prototype.drawLeaf = function(ptr,y) {
	...
	let sx = -1;
	for (let i = 0, len = this.maxkey; i < len; i++) {
		if (ptr !== null && ptr == this.leaf && i == this.item) {
			sx = x;
		}
